...
def generate_images(model, test_input, tar, i):
    ...
    generated_show = np.exp(generated)

    # visualize
    plt.figure(figsize=(3, 3), dpi=100)
    plt.title('input image')
    test_input = tf.reshape(test_input, [256, 256])
    ...
    plt.subplot(2, 1, 1)
    librosa.display.specshow(librosa.amplitude_to_db(generated_show, ref=np.max))
    plt.title('Generated spectrogram')
    plt.tight_layout()

    plt.subplot(2, 1, 2)
    plt.title('Real spectrogram')
    tar = np.reshape(tar, [256, 256])
    tar = np.exp(tar)
    librosa.display.specshow(librosa.amplitude_to_db(tar, ref=np.max))
    plt.tight_layout()
    plt.show()
...
def data_save(model, test_input, i):
    ...
    np.save("generated_signals/{}.npy".format(i), inp_spec)
...
def train_step_gen(input_spec, target, train_show, test_show, epoch):
    ...
    with tf.GradientTape() as gen_tape:
        ...
        # total loss
        gen_total_loss = gen_gan_loss + 10.0 * gen_vgg_loss + 100.0 * gen_fm_loss
